...
**Base URL:** `http://<hifiberry-ip>:1082/api` (direct access)
**Web UI URL:** `http://<hifiberry-ip>/btaudio/` (via nginx proxy)
**API via nginx:** `http://<hifiberry-ip>/api/btaudio/` (proxied)
...
The HiFiBerry Bluetooth Audio system includes:
...
```bash
# Install the btaudio package (includes web API)
sudo apt install hifiberry-btaudio
...
# Install webui package for nginx proxy (if not already installed)
sudo apt install hifiberry-webui
```

**Note:** The web API is automatically integrated with the HiFiBerry web interface through nginx proxy configuration. This provides secure external access while keeping the API service bound to localhost.
...
```javascript
// Initialize API client (using nginx proxy)
```javascript
const api = new HiFiBerryBluetoothAPI('http://your-hifiberry/api/btaudio');

// Or direct connection (local network only)
...
```bash
# Via nginx proxy
curl -X GET http://192.168.1.100/api/btaudio/status

# Direct connection (local only)
curl -X GET http://localhost:1082/api/status
```

#### Generate PIN and Make Discoverable
```bash
# Generate PIN (via nginx)
curl -X POST http://192.168.1.100/api/btaudio/pairing/pin/generate

# Make discoverable for 2 minutes (via nginx)
curl -X POST http://192.168.1.100/api/btaudio/discoverable/start \
...
```bash
# Via nginx proxy
curl -X POST http://192.168.1.100/api/btaudio/pairing/pin/set \
  -H "Content-Type: application/json" \
  -d '{"pin": "1234"}'
```
...
```bash
# List paired devices (via nginx)
curl -X GET http://192.168.1.100/api/btaudio/devices/paired

# Remove specific device (via nginx)
curl -X DELETE http://192.168.1.100/api/btaudio/devices/AA:BB:CC:DD:EE:FF

# Remove all devices (via nginx)
curl -X DELETE http://192.168.1.100/api/btaudio/devices/all
```
...
# Usage
api = HiFiBerryAPI('http://192.168.1.100/api/btaudio')
...
function BluetoothManager({ hifiberryIP }) {
    const [api] = useState(() => 
        new HiFiBerryBluetoothAPI(`http://${hifiberryIP}/api/btaudio`)
    );
...
export default {
    ...
    mounted() {
        this.api = new HiFiBerryBluetoothAPI(`http://${this.hifiberryIP}/api/btaudio`);
        this.startPolling();
    },
    methods: {
        startPolling() {
...
- **Nginx config:** `/etc/nginx/hifiberry-btaudio.nginx`
...
[Service]
ExecStart=
ExecStart=/usr/bin/btaudio-web-api --host 127.0.0.1 --port 1082
...
The API service runs on `127.0.0.1:1082` by default for security. External access is provided through nginx proxy:

- **Direct API:** `http://localhost:1082/api/` (local only)
- **Web Interface:** `http://your-hifiberry/btaudio/` (via nginx)
- **Proxied API:** `http://your-hifiberry/api/btaudio/` (via nginx)

### Security Considerations

1. **Local Binding:** The API binds to localhost only by default
2. **Nginx Proxy:** External access is controlled through nginx configuration
3. **Firewall:** Additional restrictions can be applied at the nginx level
4. **HTTPS:** Use nginx with SSL/TLS for production environments
5. **PIN Storage:** PIN codes are stored securely in `/etc/hifiberry/btaudio.conf`
...
For production environments, consider restricting origins through nginx configuration.
...
4. Test via nginx: `curl http://localhost/api/btaudio/status`
...
4. Ensure main btaudio service is running: `sudo systemctl status hifiberry-btaudio`
...
ls -la /etc/nginx/hifiberry-btaudio.nginx
```
...
```bash
# Test each endpoint via nginx proxy
curl -v http://localhost/api/btaudio/status
curl -v http://localhost/api/btaudio/audio/check
curl -v http://localhost/api/btaudio/devices/paired

# Test direct connection (local only)
